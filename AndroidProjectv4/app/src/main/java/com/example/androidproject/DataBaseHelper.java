...
class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean isScheduleAvailableForInstructor(String instructorEmail, String schedule) {
        SQLiteDatabase db = this.getReadableDatabase();

        String query = "SELECT COUNT(*) FROM AvailableCourses " +
                "INNER JOIN Courses ON AvailableCourses.course_id = Courses.id " +
                "WHERE AvailableCourses.instructor_email = ? " +
                "AND AvailableCourses.schedule = ? " +
                "AND Courses.availability = 1";
        String[] selectionArgs = {instructorEmail, schedule};
    ...
    public boolean deleteCourse(String id) {
        ...
        int availableCoursesResult = db.delete("AvailableCourses", "course_id = ?", new String[]{id});
        int applicationsResult = db.delete("Applications", "course_id = ?", new String[]{id});
        int registrationsResult = db.delete("Registrations", "course_id = ?", new String[]{id});
        db.close();
        // If all deletions were successful, it will return true
        return result != 0 && availableCoursesResult != 0 && applicationsResult != 0 && registrationsResult != 0;
    }
    ...
    public List<Pair<String, String>> getLastFiveCourses() {
        ...
        if (cursor.moveToLast()) {
            do {
                ...
                String insName = "By: " + instructorName;
                courses.add(new Pair<>(courseName, insName));
    ...
    public List<Pair<String, String>> getInstructorPreviousCourses(String email) {
        ...
        if (cursor.moveToLast()) {
            do {
                String courseName = cursor.getString(0); // get course name
                String EndDate = "Ended on:" + cursor.getString(1); // get end date
    ...
    public List<Pair<String, String>> getInstructorMondayCourses(String email) {
        ...
        if (cursor.moveToLast()) {
            do {
                String courseName = cursor.getString(0); // get course name
                String venue = "Venue: " + cursor.getString(1);
    ...
    public List<Pair<String, String>> sortCoursesByStartDate(List<Pair<String, String>> courses) {
        Collections.sort(courses, new Comparator<Pair<String, String>>() {
            ...
            public int compare(Pair<String, String> course1, Pair<String, String> course2) {
                ...
                return compareDates(startDate1, startDate2);
            }
    ...
    public void updateCourseAvailability() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                Log.d("compareTo", deadlineDate +" "+currentDate+" " + isExpired);
    ...
    private boolean compareDates(String deadlineDate, String currentDate) {
        String[] parts1 = deadlineDate.split("-");
